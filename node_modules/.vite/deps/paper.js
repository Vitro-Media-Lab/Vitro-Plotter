...
//#region (ignored) node_modules/paper/dist/node/self.js
var require_self = /* @__PURE__ */ __commonJSMin((() => {}));
//#endregion
//#region (ignored) node_modules/paper/dist/node/extend.js
var require_extend = /* @__PURE__ */ __commonJSMin((() => {}));
//#endregion
//#region node_modules/paper/dist/paper-full.js
var require_paper_full = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(self, undefined) {
		self = self || require_self();
				...
				equals: function(obj1, obj2) {
					...
					if (obj1 && obj2 && typeof obj1 === "object" && typeof obj2 === "object") {
						if (Array.isArray(obj1) && Array.isArray(obj2)) {
							...
							while (length--) if (!Base.equals(obj1[length], obj2[length])) return false;
						} else {
					...
					return Base.deserialize(typeof json === "string" ? JSON.parse(json) : json, function(ctor, args, isRoot) {
						...
						if (args.length === 1 && obj instanceof Item && (useTarget || !(obj instanceof Layer))) {
							...
							if (Base.isPlainObject(arg)) {
								arg.insert = false;
								if (useTarget) args = args.concat([Item.INSERT]);
							}
						}
		...
		var PaperScope = Base.extend({
			...
			initialize: function PaperScope() {
				...
				if (!this.agent) {
					var user = self.navigator.userAgent.toLowerCase(), os = (/(darwin|win|mac|linux|freebsd|sunos)/.exec(user) || [])[0], platform = os === "darwin" ? "mac" : os, agent = proto.agent = proto.browser = { platform };
			...
			execute: function(code, options) {
				var exports$1 = paper.PaperScript.execute(code, this, options);
				View.updateFocus();
				return exports$1;
			},
		...
		var Point = Base.extend({
			...
			getDistance: function() {
				var args = arguments, point = Point.read(args), x = point.x - this.x, y = point.y - this.y, d = x * x + y * y;
				return Base.read(args) ? d : Math.sqrt(d);
			},
		...
		var Rectangle = Base.extend({
			...
			getPoint: function(_dontLink) {
				return new (_dontLink ? Point : LinkedPoint)(this.x, this.y, this, "setPoint");
			},
			setPoint: function() {
				var point = Point.read(arguments);
				this.x = point.x;
			...
			getSize: function(_dontLink) {
				return new (_dontLink ? Size : LinkedSize)(this.width, this.height, this, "setSize");
			},
			_fw: 1,
			_fh: 1,
			setSize: function() {
			...
			setLeft: function(left) {
				if (!this._fw) {
					...
					this.width -= this._sx === .5 ? amount * 2 : amount;
				}
				this.x = left;
				this._sx = this._fw = 0;
			},
			...
			setTop: function(top) {
				if (!this._fh) {
					...
					this.height -= this._sy === .5 ? amount * 2 : amount;
				}
				this.y = top;
				this._sy = this._fh = 0;
			},
			...
			setCenterX: function(x) {
				if (this._fw || this._sx === .5) this.x = x - this.width / 2;
				else {
			...
			setCenterY: function(y) {
				if (this._fh || this._sy === .5) this.y = y - this.height / 2;
				else {
			...
			getCenter: function(_dontLink) {
				return new (_dontLink ? Point : LinkedPoint)(this.getCenterX(), this.getCenterY(), this, "setCenter");
			},
			setCenter: function() {
				var point = Point.read(arguments);
			...
			this[get] = function(_dontLink) {
				return new (_dontLink ? Point : LinkedPoint)(this[getX](), this[getY](), this, set);
			};
			this[set] = function() {
				var point = Point.read(arguments);
				this[setX](point.x);
		...
		var Matrix = Base.extend({
			...
			initialize: function Matrix(arg, _dontNotify) {
				...
				else if (count === 1 || count === 2) if (arg instanceof Matrix) this._set(arg._a, arg._b, arg._c, arg._d, arg._tx, arg._ty, _dontNotify);
				else if (Array.isArray(arg)) this._set.apply(this, _dontNotify ? arg.concat([_dontNotify]) : arg);
				else ok = false;
			...
			_changed: function() {
				var owner = this._owner;
				if (owner) if (owner._applyMatrix) owner.transform(null, true);
		...
		var Item = Base.extend(Emitter, {
			...
			return Base.each(handlers, function(name) {
				...
				statics: { _itemHandlers: handlers }
			});
		}(), {
			...
			getLayer: function() {
				var parent = this;
				while (parent = parent._parent) if (parent instanceof Layer) return parent;
				return null;
			},
			getParent: function() {
				return this._parent;
			...
			setIndex: function(index) {
				...
				if (children) parent.insertChildren(index in children ? index : undefined, [this]);
			},
			...
			rasterize: function(arg0, arg1) {
				...
				var bounds = this.getStrokeBounds(), scale = (resolution || this.getView().getResolution()) / 72, topLeft = bounds.getTopLeft().floor(), boundsSize = new Size(bounds.getBottomRight().ceil().subtract(topLeft)), rasterSize = boundsSize.multiply(scale);
			...
			_contains: function(point) {
				var children = this._children;
				if (children) {
					for (var i = children.length - 1; i >= 0; i--) if (children[i].contains(point)) return true;
					return false;
				}
			...
			matches: function(name, compare) {
				...
				if (type === "object") {
					for (var key in name) if (name.hasOwnProperty(key) && !this.matches(key, name[key])) return false;
					return true;
			...
			statics: { _getItems: function _getItems(item, options, matrix, param, firstOnly) {
				if (!param) {
			...
			isEmpty: function(recursively) {
				...
				if (recursively) {
					for (var i = 0; i < numChildren; i++) if (!children[i].isEmpty(recursively)) return false;
					return true;
				}
				return !numChildren;
			},
			...
			_getOrder: function(item) {
				...
				for (var i = 0, l = Math.min(list1.length, list2.length); i < l; i++) if (list1[i] != list2[i]) return list1[i]._index < list2[i]._index ? 1 : -1;
				return 0;
			},
			...
			isDescendant: function(item) {
				var parent = this;
				while (parent = parent._parent) if (parent === item) return true;
				return false;
			},
			...
			_setStyles: function(ctx, param, viewMatrix) {
				...
				if (style.hasStroke()) {
					...
					if (paper.support.nativeDash) {
						...
						if (dashArray && dashArray.length) if ("setLineDash" in ctx) {
		...
		var Group = Item.extend({
			...
			_draw: function(ctx, param) {
				...
				} else if (clipItem) clipItem.draw(ctx, param.extend({ clip: true }));
				var children = this._children;
		...
		var Shape = Item.extend({
				...
				if (hasFill || hasStroke || dontPaint) {
					...
					if (untransformed && isCircle) ctx.arc(0, 0, radius, 0, Math.PI * 2, true);
						...
						if (untransformed && type === "rectangle" && rx === 0 && ry === 0) ctx.rect(-width / 2, -height / 2, width, height);
							...
							var x = width / 2, y = height / 2, kappa = .44771525016920644, cx = rx * kappa, cy = ry * kappa, c = [
								-x,
								-y + ry,
								-x,
								-y + cy,
			...
			function getCornerCenter(that, point, expand) {
				...
				if (!radius.isZero()) {
					...
					for (var q = 1; q <= 4; q++) {
						var dir = new Point(q > 1 && q < 4 ? -1 : 1, q > 2 ? -1 : 1), corner = dir.multiply(halfSize), center = corner.subtract(dir.multiply(radius));
						if (new Rectangle(expand ? corner.add(dir.multiply(expand)) : corner, center).contains(point)) return {
							point: center,
							quadrant: q
						};
					}
			...
			return {
				_contains: function _contains(point) {
					...
					} else return point.divide(this.size).getLength() <= .5;
				},
		...
		var Raster = Item.extend({
			...
			setSize: function(_size, _clear) {
				var size = Size.read(arguments);
				if (!size.equals(this._size)) if (size.width > 0 && size.height > 0) {
			...
			getAverageColor: function(object) {
				...
				if (!object) bounds = this.getBounds();
					...
					else if ("x" in object) bounds = new Rectangle(object.x - .5, object.y - .5, 1, 1);
				}
				...
				ctx.restore();
				var pixels = ctx.getImageData(.5, .5, Math.ceil(width), Math.ceil(height)).data, channels = [
					0,
					0,
					0
				], total = 0;
		...
		var HitResult = Base.extend({
			...
			statics: { getOptions: function(args) {
		...
		var Segment = Base.extend({
			...
			initialize: function Segment(arg0, arg1, arg2, arg3, arg4, arg5) {
				...
				if (count > 0) if (arg0 == null || typeof arg0 === "object") if (count === 1 && arg0 && "point" in arg0) {
					point = arg0.point;
			...
			smooth: function(options, _first, _last) {
				...
				if (!type || type === "catmull-rom") {
					var a = factor === undefined ? .5 : factor, d1_a = Math.pow(d1, a), d1_2a = d1_a * d1_a, d2_a = Math.pow(d2, a), d2_2a = d2_a * d2_a;
					if (!_first && prev) {
					...
					if (prev && next) {
						var vector = p0.subtract(p2), t = factor === undefined ? .4 : factor, k = t * d1 / (d1 + d2);
		...
		var Curve = Base.extend({
			...
			divide: function(offset, isTime) {
				return this.divideAtTime(offset === undefined ? .5 : isTime ? offset : this.getTimeAt(offset));
			},
			split: function(offset, isTime) {
				return this.splitAtTime(offset === undefined ? .5 : isTime ? offset : this.getTimeAt(offset));
			},
			reversed: function() {
			...
			statics: {
				...
				getTimeOf: function(v, point) {
					var p0 = new Point(v[0], v[1]), p3 = new Point(v[6], v[7]), epsilon = 1e-12, geomEpsilon = 1e-7;
					if ((point.isClose(p0, epsilon) ? 0 : point.isClose(p3, epsilon) ? 1 : null) === null) {
				...
				getNearestTime: function(v, point) {
					if (Curve.isStraight(v)) {
						...
						return u < 1e-12 ? 0 : u > .999999999999 ? 1 : Curve.getTimeOf(v, new Point(x0 + u * vx, y0 + u * vy));
					}
				...
				_addBounds: function(v0, v1, v2, v3, coord, padding, min, max, roots) {
					...
					if (v0 < minPad || v1 < minPad || v2 < minPad || v3 < minPad || v0 > maxPad || v1 > maxPad || v2 > maxPad || v3 > maxPad) if (v1 < v0 != v1 < v3 && v2 < v0 != v2 < v3) {
						add(v0, 0);
						add(v3, 0);
					} else {
			...
			this[name] = function() {
				...
				if (!bounds) bounds = this._bounds[name] = Path[name]([this._segment1, this._segment2], false, this._path);
				return bounds.clone();
			};
		}, {}), Base.each({
			...
			}, { statics: { _evaluateMethods: methods } });
		}(), new function() {
			...
			return { statics: {
				classify: function(v) {
					var x0 = v[0], y0 = v[1], x1 = v[2], y1 = v[3], x2 = v[4], y2 = v[5], x3 = v[6], y3 = v[7], a1 = x0 * (y3 - y2) + y0 * (x2 - x3) + x3 * y2 - y3 * x2, a2 = x1 * (y0 - y3) + y1 * (x3 - x0) + x0 * y3 - y0 * x3, d3 = 3 * (x2 * (y1 - y0) + y2 * (x0 - x1) + x1 * y0 - y1 * x0), d2 = d3 - a2, d1 = d2 - a2 + a1, l = Math.sqrt(d1 * d1 + d2 * d2 + d3 * d3), s = l !== 0 ? 1 / l : 0, isZero = Numerical.isZero, serpentine = "serpentine";
					d1 *= s;
					d2 *= s;
					d3 *= s;
			...
			function addCurveIntersections(v1, v2, c1, c2, locations, include, flip, recursion, calls, tMin, tMax, uMin, uMax) {
				...
				if (Math.max(uMax - uMin, tMaxNew - tMinNew) < fatLineEpsilon) {
					...
					var uDiff = uMax - uMin;
					if (tMaxClip - tMinClip > .8) if (tMaxNew - tMinNew > uDiff) {
						var parts = Curve.subdivide(v1, .5), t = (tMinNew + tMaxNew) / 2;
						...
						var parts = Curve.subdivide(v2, .5), u = (uMin + uMax) / 2;
		...
		var CurveLocation = Base.extend({
			...
			equals: function(loc, _ignoreOther) {
				...
				if (!res && loc instanceof CurveLocation) {
					var c1 = this.getCurve(), c2 = loc.getCurve(), p1 = c1._path;
					if (p1 === c2._path) {
			...
			function insert(locations, loc, merge) {
				...
				while (l <= r) {
					...
					if ((path1 !== path2 ? path1._id - path2._id : loc.getIndex() + loc.getTime() - (loc2.getIndex() + loc2.getTime())) < 0) r = m - 1;
					else l = m + 1;
				}
		...
		var PathItem = Item.extend({
			...
			statics: { create: function(arg) {
				...
				return new (compound ? CompoundPath : Path)(arg);
			} },
			_asPathItem: function() {
				return this;
			},
			...
			setPathData: function(data) {
				var parts = data && data.match(/[mlhvcsqtaz][^mlhvcsqtaz]*/gi), coords, relative = false, previous, control, current = new Point(), start = new Point();
		...
		var Path = PathItem.extend({
			...
			hasHandles: function() {
				...
				for (var i = 0, l = segments.length; i < l; i++) if (segments[i].hasHandles()) return true;
				return false;
			},
			clearHandles: function() {
				var segments = this._segments;
			...
			_updateSelection: function(segment, oldSelection, newSelection) {
				...
				if ((this._segmentSelection += newSelection - oldSelection) > 0) this.setSelected(true);
			},
			divideAt: function(location) {
			...
			flatten: function(flatness) {
				var parts = new PathFlattener(this, flatness || .25, 256, true).parts, length = parts.length, segments = [];
			...
			smooth: function(options) {
				...
				if (from > to) if (closed) from -= length;
				else {
					var tmp = from;
					from = to;
					to = tmp;
			...
			toShape: function(insert) {
				...
				function isArc(i) {
					var seg1 = segments[i], seg2 = seg1.getNext(), handle1 = seg1._handleOut, handle2 = seg2._handleIn, kappa = .5522847498307936;
			...
			_hitTestSelf: function(point, options, viewMatrix, strokeMatrix) {
				...
				if (strokeRadius !== null) if (strokeRadius > 0) {
				...
				_draw: function(ctx, param, viewMatrix, strokeMatrix) {
					...
					if (!dontPaint && (hasFill || hasStroke)) {
						...
						if (hasStroke) {
							if (dashLength) {
								if (!dontStart) ctx.beginPath();
								var flattener = new PathFlattener(this, .25, 32, false, strokeMatrix), length = flattener.length, from = -style.getDashOffset(), to, i = 0;
			...
			return {
				...
				curveTo: function() {
					var args = arguments, through = Point.read(args), to = Point.read(args), t = Base.pick(Base.read(args), .5), t1 = 1 - t, current = getCurrentSegment(this)._point, handle = through.subtract(current.multiply(t1 * t1)).subtract(to.multiply(t * t)).divide(2 * t * t1);
			...
			_getBounds: function(matrix, options) {
				return Path[options.handle ? "getHandleBounds" : options.stroke ? "getStrokeBounds" : "getBounds"](this._segments, this._closed, this, matrix, options);
			},
			statics: {
		...
		Path.inject({ statics: new function() {
			var kappa = .5522847498307936, ellipseSegments = [
			...
			return {
				...
				Circle: function() {
					var args = arguments;
					return createEllipse(Point.readNamed(args, "center"), new Size(Base.readNamed(args, "radius")), args);
				},
				Rectangle: function() {
				...
				RegularPolygon: function() {
					var args = arguments, center = Point.readNamed(args, "center"), sides = Base.readNamed(args, "sides"), radius = Base.readNamed(args, "radius"), step = 360 / sides, three = sides % 3 === 0, vector = new Point(0, three ? -radius : radius), offset = three ? -1 : .5, segments = new Array(sides);
		...
		var CompoundPath = PathItem.extend({
			...
			initialize: function CompoundPath(arg) {
				this._children = [];
				this._namedChildren = {};
				if (!this._initialize(arg)) if (typeof arg === "string") this.setPathData(arg);
			...
			isClosed: function() {
				var children = this._children;
				for (var i = 0, l = children.length; i < l; i++) if (!children[i]._closed) return false;
				return true;
			},
			setClosed: function(closed) {
				var children = this._children;
		...
		PathItem.inject(new function() {
			...
			function getWinding(point, curves, dir, closed, dontFlip) {
				...
				function handleCurve(v) {
					...
					if (po <= max(o0, o1, o2, o3) && po >= min(o0, o1, o2, o3)) {
						var a0 = v[ia + 0], a1 = v[ia + 2], a2 = v[ia + 4], a3 = v[ia + 6], monoCurves = paL > max(a0, a1, a2, a3) || paR < min(a0, a1, a2, a3) ? [v] : Curve.getMonoCurves(v, dir), res;
						for (var i = 0, l = monoCurves.length; i < l; i++) if (res = addWinding(monoCurves[i])) return res;
					}
				}
			...
			function propagateWinding(segment, path1, path2, curveCollisionsMap, operator) {
				...
				var offsets = [
					...
					winding: 0,
					quality: -1
				}, tMin = .001, tMax = 1 - tMin;
				for (var i = 0; i < offsets.length && winding.quality < .5; i++) {
					...
					for (var j = 0, l = chain.length; j < l; j++) {
						...
						if (length <= curveLength) {
							...
							if (operator.subtract && path2) {
								var pathWinding = (operand === path1 ? path2 : path1)._getWinding(pt, dir, true);
								if (operand === path1 && pathWinding.winding || operand === path2 && !pathWinding.winding) if (pathWinding.quality < 1) continue;
								else wind = {
									winding: 0,
									quality: 1
								};
			...
			function tracePaths(segments, operator) {
				...
				function isStart(seg) {
					if (seg) {
						for (var i = 0, l = starts.length; i < l; i++) if (seg === starts[i]) return true;
					}
					return false;
				}
			...
			return {
				...
				getInteriorPoint: function() {
					var point = this.getBounds().getCenter(true);
		...
		var PathFlattener = Base.extend({
			...
			initialize: function(path, flatness, maxRecursion, ignoreStraight, matrix) {
				...
				function computeParts(curve, index, t1, t2) {
					if (t2 - t1 > minSpan && !(ignoreStraight && Curve.isStraight(curve)) && !Curve.isFlatEnough(curve, flatness || .25)) {
						var halves = Curve.subdivide(curve, .5), tMid = (t1 + t2) / 2;
			...
			_get: function(offset) {
				var parts = this.parts, length = parts.length, i, j = this.index;
				for (;;) {
					i = j;
		...
		var PathFitter = Base.extend({
			...
			addCurve: function(segments, curve) {
				segments[segments.length - 1].setHandleOut(curve[1].subtract(curve[0]));
			...
			findMaxError: function(first, last, curve, u) {
				...
				for (var i = first + 1; i < last; i++) {
					var v = this.evaluate(3, curve, u[i - first]).subtract(this.points[i]);
		...
		var PointText = TextItem.extend({
			...
			_getBounds: function(matrix, options) {
				...
				var rect = new Rectangle(x, numLines ? -.75 * leading : 0, width, numLines * leading);
		...
		var Color = Base.extend(new function() {
			...
			var componentParsers = {}, namedColors = { transparent: [
				0,
				0,
				0,
				0
			...
			function fromCSS(string) {
				...
				if (match) {
					...
					if (!color) if (window) {
						if (!colorCtx) {
							colorCtx = CanvasProvider.getContext(1, 1, { willReadFrequently: true });
			...
			var converters = {
				"rgb-hsb": function(r, g, b) {
					var max = Math.max(r, g, b), delta = max - Math.min(r, g, b);
					return [
						delta === 0 ? 0 : (max == r ? (g - b) / delta + (g < b ? 6 : 0) : max == g ? (b - r) / delta + 2 : (r - g) / delta + 4) * 60,
						max === 0 ? 0 : delta / max,
						max
					];
				},
				...
				"rgb-hsl": function(r, g, b) {
					var max = Math.max(r, g, b), min = Math.min(r, g, b), delta = max - min, achromatic = delta === 0, h = achromatic ? 0 : (max == r ? (g - b) / delta + (g < b ? 6 : 0) : max == g ? (b - r) / delta + 2 : (r - g) / delta + 4) * 60, l = (max + min) / 2;
					return [
						h,
						achromatic ? 0 : l < .5 ? delta / (max + min) : delta / (2 - max - min),
						l
					];
				},
				"hsl-rgb": function(h, s, l) {
					...
					var t3s = [
						...
						h,
						h - 1 / 3
					], t2 = l < .5 ? l * (1 + s) : l + s - l * s, t1 = 2 * l - t2, c = [];
					for (var i = 0; i < 3; i++) {
						var t3 = t3s[i];
						if (t3 < 0) t3 += 1;
						if (t3 > 1) t3 -= 1;
			...
			return Base.each(types, function(properties, type) {
				...
				initialize: function Color(arg) {
					...
					if (!components) {
						...
						} else if (argType === "object") if (arg.constructor === Color) {
							type = arg._type;
				...
				_changed: function() {
					...
					if (this._owner) if (this._setter) this._owner[this._setter](this);
			...
			equals: function(gradient) {
				...
				if (gradient && this._class === gradient._class) {
					...
					if (length === stops2.length) {
						for (var i = 0; i < length; i++) if (!stops1[i].equals(stops2[i])) return false;
						return true;
					}
				}
				return false;
		...
		var Style = Base.extend(new function() {
			var itemDefaults = {
				...
				leading: null,
				justification: "left"
			}), textDefaults = Base.set({}, groupDefaults, { fillColor: new Color() }), flags = {
				strokeWidth: 193,
				strokeCap: 193,
				strokeJoin: 193,
				strokeScaling: 201,
				...
				fields[get] = function(_dontMerge) {
					...
					if (applyToChildren && !_dontMerge) for (var i = 0, l = children.length; i < l; i++) {
						...
						if (value === undefined) {
							...
							if (ctor && !(value && value.constructor === ctor)) this._values[key] = value = ctor.read([value], 0, {
								readNull: true,
								clone: true
							});
						}
		...
		DomEvent.requestAnimationFrame = new function() {
			...
			return function(callback) {
				...
				} else if (!timer) timer = setInterval(handleCallbacks, 1e3 / 60);
			};
		}();
		var View = Base.extend(Emitter, {
			...
			create: function(project, element) {
				...
				return new (window ? CanvasView : View)(project, element);
			}
		} }, new function() {
			if (!window) return;
			...
			var viewEvents = {}, docEvents = {
				mouseout: function(event) {
					...
					if (view && (!target || target.nodeName === "HTML")) {
						var offset = DomEvent.getOffset(event, view._element), x = offset.x, abs = Math.abs, ax = abs(x), diff = ax - (1 << 25);
			...
			var called = false, prevented = false, fallbacks = {
				doubleclick: "click",
				mousedrag: "mousemove"
			}, wasInView = false, downPoint, lastPoint, downItem, overItem, dragItem, clickItem, clickTime, dblClick;
			...
			return {
				...
				statics: {
					...
					_resetState: function() {
						...
						prevFocus = tempFocus = downPoint = lastPoint = downItem = overItem = dragItem = clickItem = clickTime = dblClick = null;
					}
				}
			};
		}());
		...
		var Key = new function() {
			...
			function handleKey(down, key, character, event) {
				...
				if (key.length > 1 && (name = Base.camelize(key)) in modifiers) {
					modifiers[name] = down;
					var agent = paper && paper.agent;
					if (name === "meta" && agent && agent.mac) if (down) metaFixMap = {};
					else {
		...
		var Tween = Base.extend(Emitter, {
			...
			_parseKeys: function(keys) {
				...
				for (var i = 0, l = keys.length; i < l; i++) {
					var key = keys[i];
					parsed[key] = key.replace(/\.([^.]*)/g, "/$1").replace(/\[['"]?([^'"\]]*)['"]?\]/g, "/$1").split("/");
				}
				return parsed;
			},
		...
		var Http = { request: function(options) {
		...
		new function() {
			...
			function exportGroup(item, options) {
				...
				for (var i = 0, l = children.length; i < l; i++) {
					...
					if (childNode) if (child.isClipMask()) {
			...
			function exportPath(item, options) {
				...
				if (matchShapes && length >= 2 && !item.hasHandles()) if (length > 2) {
			...
			function exportShape(item) {
				...
				if (radius) if (type === "circle") attrs.r = radius;
				else {
			...
			function exportSymbolItem(item, options) {
				...
				if (!node) {
					node = SvgElement.create("symbol", { viewBox: formatter.rectangle(bounds) });
			...
			function exportGradient(color) {
				...
				if (!gradientNode) {
					...
					for (var i = 0, l = stops.length; i < l; i++) {
						...
						attrs = { offset: offset == null ? i / (l - 1) : offset };
		...
		new function() {
			...
			function importNode(node, options, isRoot) {
				...
				if (isRoot && isElement) {
					...
					container = SvgElement.create("svg", { style: "stroke-width: 1px; stroke-miterlimit: 10" });
				...
				if (container) {
					...
					if (parent) if (next) parent.insertBefore(node, next);
		...
		Base.exports.PaperScript = function() {
			...
			if (!acorn && typeof __require !== "undefined") try {
				acorn = __require("acorn");
			} catch (e) {}
			if (!acorn) {
				var exports$2, module$1;
				acorn = exports$2 = module$1 = {};
				(function(root, mod) {
					if (typeof exports$2 == "object" && typeof module$1 == "object") return mod(exports$2);
					if (typeof define == "function" && define.amd) return define(["exports"], mod);
					mod(root.acorn || (root.acorn = {}));
				})(this, function(exports$3) {
					"use strict";
					exports$3.version = "0.5.0";
					var options, input, inputLen, sourceFile;
					exports$3.parse = function(inpt, opts) {
						input = String(inpt);
					...
					var defaultOptions = exports$3.defaultOptions = {
						ecmaVersion: 5,
					...
					var getLineInfo = exports$3.getLineInfo = function(input, offset) {
					...
					exports$3.tokenize = function(inpt, opts) {
						input = String(inpt);
						inputLen = input.length;
						setOptions(opts);
						initTokenState();
					...
					for (var kw in keywordTypes) exports$3.tokTypes["_" + kw] = keywordTypes[kw];
					...
					var newline = /[\n\r\u2028\u2029]/;
					var lineBreak = /\r\n|[\n\r\u2028\u2029]/g;
					var isIdentifierStart = exports$3.isIdentifierStart = function(code) {
					...
					var isIdentifierChar = exports$3.isIdentifierChar = function(code) {
					...
					function readToken_mult_modulo() {
						if (input.charCodeAt(tokPos + 1) === 61) return finishOp(_assign, 2);
					...
					function readToken_eq_excl(code) {
						if (input.charCodeAt(tokPos + 1) === 61) return finishOp(_equality, input.charCodeAt(tokPos + 2) === 61 ? 3 : 2);
						return finishOp(code === 61 ? _eq : _prefix, 1);
					}
					function getTokenFromCode(code) {
						switch (code) {
							case 46: return readToken_dot();
							case 40:
								++tokPos;
							...
							case 55:
							case 56:
							case 57: return readNumber(false);
							case 34:
							case 39: return readString(code);
							case 47: return readToken_slash(code);
							case 37:
							case 42: return readToken_mult_modulo();
							case 124:
							case 38: return readToken_pipe_amp(code);
							case 94: return readToken_caret();
							case 43:
							case 45: return readToken_plus_min(code);
							case 60:
							case 62: return readToken_lt_gt(code);
							case 61:
							case 33: return readToken_eq_excl(code);
							case 126: return finishOp(_prefix, 1);
						}
						return false;
					}
					...
					function parseExprOp(left, minPrec, noIn) {
						...
						if (prec != null && (!noIn || tokType !== _in)) {
							if (prec > minPrec) {
								...
								return parseExprOp(finishNode(node, op === _logicalOR || op === _logicalAND ? "LogicalExpression" : "BinaryExpression"), minPrec, noIn);
							}
						}
						return left;
					}
				...
				function handleExports(node) {
					switch (node.type) {
						case "ExportDefaultDeclaration":
							replaceCode({ range: [node.start, node.declaration.start] }, "module.exports = ");
							break;
						case "ExportNamedDeclaration":
							...
							if (declaration) {
								...
								if (declarations) {
									...
									replaceCode({ range: [node.start, declaration.start + declaration.kind.length] }, "");
								}
							} else if (specifiers) {
								var exports$4 = specifiers.map(function(specifier) {
									var name = getCode(specifier);
									return "module.exports." + name + " = " + name + "; ";
								}).join("");
								if (exports$4) replaceCode(node, exports$4);
							}
							break;
					}
				}
			...
			function execute(code, scope, options) {
				paper = scope;
				var view = scope.getView(), tool = /\btool\.\w+|\s+on(?:Key|Mouse)(?:Up|Down|Move|Drag)\b/.test(code) && !/\bnew\s+Tool\b/.test(code) ? new Tool() : null, toolHandlers = tool ? tool._events : [], handlers = ["onFrame", "onResize"].concat(toolHandlers), params = [], args = [], func;
				code = (typeof code === "object" ? code : compile(code, options)).code;
				...
				var exports$5 = Base.each(handlers, function(key) {
				...
				var exports$5 = func && func.apply(scope, args);
				var obj = exports$5 || {};
			...
			if (window) if (document.readyState === "complete") setTimeout(loadAll);
			else DomEvent.add(window, { load: loadAll });
			return {
				compile,
				execute,
				load,
		...
		if (paper.agent.node) require_extend()(paper);
		if (typeof define === "function" && define.amd) define("paper", paper);
		else if (typeof module === "object" && module) module.exports = paper;
		return paper;
